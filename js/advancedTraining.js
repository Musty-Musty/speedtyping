...
function randomParagraph() {
  ...
  let randIndex = Math.floor(Math.random() * advancedParagraphs.length);
  typingText.innerHTML = "";
  // getting random item from the paragraphs array, splitting all characters
  // of it, adding each character inside a span and then adding this span inside p tag
  advancedParagraphs[randIndex].split("").forEach(span => {
...
function initTyping() {
  ...
  if(charIndex < characters.length - 1 && timeLeft > 0) {
    ...
    if(typedChar == null) {
      ...
      // increment index so it'll be 0, 1, 2, 3 as user types and increment charIndex either user typed correct or incorrect character
      charIndex++;
    }
